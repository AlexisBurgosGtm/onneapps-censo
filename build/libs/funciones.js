let f = {
    ...
    if (rangos[i][0] <= nnum && nnum <= rangos[i][1]) {
        if (hablado=='SI'){     
            f.hablar("Su línea telefónica es " + rangos[i][2]);
        }else{
            return rangos[i][2];
        }
        found = true;
    ...
    if (!found) {
    if (hablado=='SI'){ 
        f.hablar("El número indicado no aparece en la lista");
    }else{
    ...
    getComboSucursales: ()=>{
      let str = '';
      let data = [
        {codsucursal:"DISTESTRELLA000",nomsucursal:"DISTRIBUIDORA ESTRELLA",color:"warning"}
      ];
