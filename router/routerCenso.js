...
router.post("/listaclientes", async(req,res)=>{
    ...
    ME_CENSO.CODDEPTO, ME_CENSO.REFERENCIA, ME_CENSO.OBS, ME_CENSO.VISITA, ME_CENSO.LAT, ME_CENSO.LONG, ME_CENSO.TELEFONO, ME_Municipios.DESMUNI AS MUNICIPIO, ME_Departamentos.DESDEPTO AS DEPARTAMENTO, ME_CENSO.STATUS, ISNULL(ME_CENSO.SECTOR,'A') AS SECTOR
    FROM ME_CENSO LEFT OUTER JOIN
...
router.post("/editarcliente", async(req,res)=>{
    ...
    REFERENCIA='${referencia}',OBS='${obs}',VISITA='${visita}',LAT=${lat},LONG=${long}, SECTOR='${sector}'
